...
% df/dt + u*df/dx = k(x) in 1-D subject to the following boundary conditions.
...
function [f, varargout] = model_3(N_cells, f0fun, u, kfun,kprimefun, CFL, scheme, t_vec, x_vec, varargin)
%% Description
...
%kfun: Provide the nonhomogeneous term as a function handle.
%kprimefun: Provide dg/dx as a function handle. For the Lax Wendroff scheme
...
while t < t_vec(2) - dt*1e-3
   if scheme == "Upwind"
       %update first node
       f_new(1) = f_old(1) - CFL*f_old(1) + dt*k(1);
       %update the rest
       for i = 2:length(mesh)
   ...
   elseif scheme == "Leapfrog"
       ...
       if counter == 1
           ...
           %update the rest
           for i = 2:length(mesh) -1
              f_new(i) = f_old_old(i) - CFL*(f_old(i+1) - f_old(i-1)) + 2*dt*k(i);
           end
           %update last node
